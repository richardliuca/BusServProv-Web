...
const Footer = () => (
  <footer id="location" className="bg-cocoa text-linen">
    <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-20">
      <div className="grid gap-12 md:grid-cols-3">
        <div>
          <Logo onDark />
          <p className="mt-4 max-w-xs text-sm leading-relaxed text-linen/60">
            A local Chinatown mom &amp; pop spa and massage studio. Come relieve
        ...
        <div>
          ...
          <address className="mt-4 text-base not-italic leading-relaxed text-linen/80">
            {AppConfig.addressLine1}
            <br />
            {AppConfig.addressLine2}
          </address>
          <a
            href={AppConfig.googleMapsUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-3 inline-flex items-center gap-1.5 text-sm font-medium text-primary-300 transition-colors hover:text-primary-200"
          >
            Get directions
            <svg
        ...
        <div>
          ...
          <a
            ...
            className="mt-4 inline-block text-2xl font-semibold text-linen transition-colors hover:text-primary-300"
          >
            {AppConfig.phoneDisplay}
          </a>
          <p className="mt-2 text-sm text-linen/60">
            Call or text — walk-ins welcome.
          </p>
          ...
          <div className="mt-6 flex gap-3">
            {SOCIAL_LINKS.map((link) => (
              <a
                ...
                className="flex size-11 items-center justify-center rounded-full border border-linen/15 text-linen/70 transition-all hover:border-primary-400 hover:bg-primary-400/10 hover:text-primary-300 [&_svg]:size-5 [&_svg]:fill-current"
              >
                {link.icon}
              </a>
            ))}
      ...
      <div className="mt-14 border-t border-linen/10 pt-8 text-center text-sm text-linen/50">
